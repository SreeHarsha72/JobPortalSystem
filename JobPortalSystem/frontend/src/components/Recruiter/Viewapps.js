...
export default class Viewapps extends Component {
  ...
  onsort(e) {
    ...
    axios
      .get(
        "http://localhost:4000/application/job/" + localStorage.getItem("job")
      )
      .then((res) => {
        ...
        applications.forEach((app) => {
          ...
          v.sop = app.sop;
          axios
            .post("http://localhost:4000/applicant/email", {
              email: app.applicant,
            })
            .then((user) => {
              if (user.data) {
                ...
                if (this.state.sort === 1) {
                  ...
                  d.sort(function (a, b) {
                    if (
                      a.applicant.name != undefined &&
                      b.applicant.name != undefined
                    ) {
                      return (
                        (1 - flag * 2) * (a.applicant.name - b.applicant.name)
                      );
                    } else {
                      return 1;
                    }
                  ...
                  d.sort(function (a, b) {
                    if (
                      a.applicant.rating != undefined &&
                      b.applicant.rating != undefined
                    ) {
                      return (
                        (1 - flag * 2) *
                        (a.applicant.rating - b.applicant.rating)
                      );
                    } else {
                      return 1;
                    }
  ...
  onReject(e) {
    // console.log("LOL");
    axios
      .post("http://localhost:4000/application/update/" + e, {
        status: "Reject",
      })
      .then((res) => {
