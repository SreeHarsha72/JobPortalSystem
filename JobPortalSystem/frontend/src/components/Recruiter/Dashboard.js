...
export default class Recruiterhome extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .post("http://localhost:4000/recruiter/email", {
        email: localStorage.getItem("email"),
      })
      .then((response) => {
  ...
  onChangebio(event) {
    ...
    let c = s.split(" ").filter(function (str) {
      return str != "";
    }).length;
  ...
  onUpdate(e) {
    ...
    console.log(data);
    axios
      .post("http://localhost:4000/recruiter/update", data)
      .then((res) => {
        console.log("response upd rec", res);
        if (res.data.status !== "empty") alert("updated user info");
      })
      .catch(function (error) {
        console.log(error);
      });
        ...
        <Navbar collapseOnSelect expand="lg" bg="dark" variant="dark">
          ...
          <Navbar.Collapse id="responsive-navbar-nav">
            <Nav className="mr-auto">
              <Nav.Link href="/recruiter/dashboard" className="active">
                Home
              </Nav.Link>
